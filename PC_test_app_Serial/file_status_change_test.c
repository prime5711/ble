...
//  	file_fd =  open(argv[1], O_RDWR | O_CREAT | O_NONBLOCK | O_SYNC | O_TRUNC, S_IRWXU| S_IRGRP| S_IXGRP | S_IROTH | S_IXOTH );
//  	file_fd =  open(argv[1], O_RDWR | O_NONBLOCK | O_SYNC | O_TRUNC, S_IRWXU| S_IRGRP| S_IXGRP | S_IROTH | S_IXOTH );
	file_fd = open(argv[1], O_RDWR | O_NONBLOCK | O_NDELAY);
...
//  
//  
//  	tcflush(file_fd, TCIOFLUSH);
//  	tcsetattr(file_fd, TCSANOW, &newtio);
//  
//  	gStatus = STATUS_INIT;
//  	write(file_fd,RFID_SET_AUTODETECT,sizeof(RFID_SET_AUTODETECT));
	...
	while(loop_stop == NO)
	{
		...
		select_ret = select(file_fd+1, &rfds, NULL, NULL, &tv);
...
//  					memset(stdin_buf,0,sizeof(stdin_buf));
			...
			if( FD_ISSET(file_fd,&rfds) )
			{
				char test_str[5] ="test";
				test_str[4] = 0 ;
				printf("file_status changed\n");
				
				fcntl(file_fd,F_SETFL,0);
				fstat(file_fd, &f_stat);
				ret = read(file_fd, f_buf, 5);
				lseek(file_fd, 0, SEEK_SET);

				write(file_fd,test_str,sizeof(test_str));
				if( ret == 0 ) 
					continue;
				printf("packet first(%d)= ",ret);
				for(i = 0 ; i < ret ; i++ )
					printf("%02x ", f_buf[i] );
				printf("\n");
			}
...
//  	tcsetattr(file_fd, TCSANOW, &oldtio);
	close(file_fd);
