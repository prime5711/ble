...
struct _card_block_info_bitfield
{
	//block 0
...
struct _card_block_info
{
	...
	unsigned char day;
	unsigned char month;
	unsigned char year; // 2014년 기준
	unsigned char reserved3; // 2014년 기준

}__attribute__((packed));

int copy_card_block_info_to_nonbit(struct _card_block_info_bitfield *src, struct _card_block_info *dst)
{
	memcpy((const char*)dst->FullName,(const char*)src->FullName, (size_t)strlen(src->FullName)); 
	dst->weight 		= src->weight;
	dst->height 		= src->weight;
	dst->protein 		= src->weight;
	dst->icf 			= src->icf;
	dst->ecf 			= src->ecf;
	dst->mm 			= src->mm;
	dst->smm			= src->smm;
...
int copy_card_block_info_to_bit(struct _card_block_info *src, struct _card_block_info_bitfield *dst)
{
	memcpy((const char*)dst->FullName,(const char*)src->FullName, (size_t)strlen(src->FullName));
	dst->weight 		= src->weight;
	dst->height 		= src->weight;
...
struct _card_block_info_bitfield gCard_Block_Info;
struct _card_block_info_bitfield gCard_Block_Info_Bitfield;

void Print_Card_Info(struct _card_block_info_bitfield *p)
{
...
void Processing_UartPacket(unsigned char *buf)
{
	...
	{
		printf("Checksum Fail(Tobe:%02x) ", checksum);
	...
	switch( gStatus )
	{
		case STATUS_INIT :
			if( packet_recv.cmd == CMD_CARD_TYPE_ALL_SERIAL_READ ) 
			{
				...
				printf("Set Card Reader : Dummu Reader Mode\n");
			}
		...
		case STATUS_CARD_READ_INFO :
			...
			memcpy( &gCard_Block_Info_Bitfield, packet_recv.packet_buffer, packet_recv.packet_length);
			...
			else
			{
				...
				if( memcmp(gCard_Block_Info_Bitfield.nuga_sig, (const void*)NUGA_ID, 3) != MEM_MATCH )
				{
					fail=1;
			...
			INNO_SHCHO_PRINT("\n\n\t \033[22;30;31m line:%d:@%s in %s                \033[0m \n\n",__LINE__,__FUNCTION__,__FILE__  ); 
			Print_Card_Info(&gCard_Block_Info_Bitfield);
			...
			//-----------------------------------------------------------------------------------------------------
			#if 0 //  for test
			memcpy(gCard_Block_Info_Bitfield.FullName, "조승훈",strlen("조승훈"));
			gCard_Block_Info_Bitfield.FullName[strlen("조승훈")] = 0 ; 
			gCard_Block_Info_Bitfield.weight = 8010;
			gCard_Block_Info_Bitfield.height =17000;
			gCard_Block_Info_Bitfield.protein =1220;
			gCard_Block_Info_Bitfield.icf =2990;
			gCard_Block_Info_Bitfield.ecf =1500;
			gCard_Block_Info_Bitfield.mm = 420;
			gCard_Block_Info_Bitfield.smm= 3400;
			gCard_Block_Info_Bitfield.fat= 1010;
			gCard_Block_Info_Bitfield.trs= 2660;
			gCard_Block_Info_Bitfield.ras= 336;
			gCard_Block_Info_Bitfield.las= 334;
			gCard_Block_Info_Bitfield.rls= 999;
			gCard_Block_Info_Bitfield.lls= 1000;
			gCard_Block_Info_Bitfield.bmi= 2160;
			gCard_Block_Info_Bitfield.pbf= 1410;
			gCard_Block_Info_Bitfield.od= 9820;
			gCard_Block_Info_Bitfield.whr= 83;
			gCard_Block_Info_Bitfield.vfa= 6330;
			gCard_Block_Info_Bitfield.body_type= 1;
			gCard_Block_Info_Bitfield.body_index= 2;
			gCard_Block_Info_Bitfield.scr= 8800;
			gCard_Block_Info_Bitfield.cwe= 150;
			gCard_Block_Info_Bitfield.cfa= 150;
			gCard_Block_Info_Bitfield.minute= 42;
			gCard_Block_Info_Bitfield.hour= 13;
			gCard_Block_Info_Bitfield.day= 26;
			gCard_Block_Info_Bitfield.month= 5;
			gCard_Block_Info_Bitfield.year= 2014-2014;
			gCard_Block_Info_Bitfield.reserved3= 0;

			gCard_Block_Info_Bitfield.minute +=1 ;
			gCard_Block_Info_Bitfield.minute %= 60 ;
			#endif

			// Android Java에서 BitField를 지원하지 않는 다고 해서 BitField를 없앤 것으로 주고 받는다.
			copy_card_block_info_to_nonbit(&gCard_Block_Info_Bitfield, gCard_Block_Info);
...
//  						read(0,stdin_buf,sizeof(stdin_buf)); // just read
			...
			//-----------------------------------------------------------------------------------------------------
			// Android Java에서 BitField를 지원하지 않는 다고 해서 BitField를 없앤 것으로 주고 받는다.
			copy_card_block_info_to_bit(&gCard_Block_Info, gCard_Block_Info_Bitfield);

			f_write_fd =  open("gCard_Block_Info_Bitfield.bin", O_RDWR | O_CREAT | O_NONBLOCK | O_SYNC | O_TRUNC, S_IRWXU| S_IRGRP| S_IXGRP | S_IROTH | S_IXOTH );
			write(f_write_fd, &gCard_Block_Info_Bitfield, sizeof(struct _card_block_info_bitfield));
			close(f_write_fd);
			//-----------------------------------------------------------------------------------------------------
...
//  			hexDump((char *)&gCard_Block_Info_Bitfield.bia_2c,2);
//  			hexDump((char *)&gCard_Block_Info_Bitfield.bia_2d,2);
//  
//  			num_bia_check = swap16(gCard_Block_Info_Bitfield.bia_2d.s);
//  			num_bia_check += 1;
//  			gCard_Block_Info_Bitfield.bia_2d.s = swap16(num_bia_check);
//  			
//  			RFID_WRITE_BLOCK26[5] =  gCard_Block_Info_Bitfield.bia_2c.ch[0];
//  			RFID_WRITE_BLOCK26[6] =  gCard_Block_Info_Bitfield.bia_2c.ch[1];
//  			RFID_WRITE_BLOCK26[7] =  gCard_Block_Info_Bitfield.bia_2d.ch[0];
//  			RFID_WRITE_BLOCK26[8] =  gCard_Block_Info_Bitfield.bia_2d.ch[1];
//  
...
//  			usleep(100000);
//  
			gCard_char = (unsigned char *)&gCard_Block_Info_Bitfield;
			for( i = 0 ; i < 28; i++)
			{
